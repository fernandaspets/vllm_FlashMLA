...
// Pipeline Overview:
// SM120 CUTLASS CollectiveBuilder configuration (from 80a example)
#if defined(CUTLASS_ARCH_MMA_SM120_SUPPORTED)

using ArchTag = cutlass::arch::Sm120;
using OperatorClassSparse = cutlass::arch::OpClassBlockScaledSparseTensorOp;

// Element types for MXFP8->BF16 sparse (example 80a)
...
// Kernel schedule
using KernelScheduleSparse = cutlass::gemm::KernelSparseTmaWarpSpecializedMxf8f6f4Acc2x4Sm120;
...
// | Copy |    MMA    |   Scale | Copy |    MMA    |   Scale & Exp   | Exp   |
...
sparse_attn_fwd_kernel(__grid_constant__ const SparsePrefillParams params, __grid_constant__ const TmaParams tma_params) {
    ...
    const int warpgroup_idx = __shfl_sync(0xffffffff, threadIdx.x / 80, 0);
    ...
    if (warp_idx == 0) {
        if (elect_one_sync()) {
            ...
            for (int i = 0; i < NUM_BUFS; ++i) {
                ...
                plan.bar_so_ready[i].init(160*2);
                plan.bar_k_valid_ready[i].init(16);
                plan.bar_k_valid_free[i].init(160);
            }
            fence_barrier_init();
        }
    }
    ...
    if (warpgroup_idx == 0) {
        ...
        for (int k = 0; k < num_k_blocks; ++k) {
            ...
            NamedBarrier::arrive_and_wait(160, 0);  // Wait for rowwise_max_buf and sP to be ready
            plan.rowwise_max_buf[idx_in_warpgroup] = cur_pi_max;
            NamedBarrier::arrive_and_wait(160, 0);  // TODO Name these barriers
        ...
        NamedBarrier::arrive_and_wait(160, 0);
...
void run_fwd_kernel(const SparsePrefillParams& params) {
    ...
    FLASH_ASSERT(params.h_q % B_H == 0); // Must be divisible for multi-iteration  // To save some calculation
